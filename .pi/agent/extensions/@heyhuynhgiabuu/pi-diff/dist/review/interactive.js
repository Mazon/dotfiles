export function formatInteractiveReviewPanel(diff, comments = [], options = {}) {
    ...
    for (const file of diff.files.slice(0, maxFiles)) {
        ...
        if (file.hunks.length > 3)
            lines.push(`    - … ${file.hunks.length - 3} more hunks`);
    }
    if (diff.files.length > maxFiles)
        lines.push(`  … ${diff.files.length - maxFiles} more files`);
    lines.push("");
    if (selectedFile) {
...
export function formatReviewComments(comments) {
    const lines = [];
...
function formatFilePanel(file, comments, options, selectedHunk) {
    const lines = [];
    ...
    if (file.oldPath && file.oldPath !== file.path)
        lines.push(`Old path: ${file.oldPath}`);
    ...
    if (fileComments.length > 0)
        lines.push(`Drafted comments on this file: ${fileComments.length}`);
    lines.push("");
...
function formatHunk(file, hunk, maxLines) {
    const lines = [];
    ...
    if (hunk.lines.length > maxLines)
        lines.push(` … ${hunk.lines.length - maxLines} more lines omitted`);
    lines.push("```");
    lines.push("");
    lines.push(`File: ${file.path}, hunk: ${hunk.id}, first changed line: ${firstChangedLine(hunk) ?? hunk.newStart}`);
    return lines.join("\n");
}
...
function findFile(diff, path) {
    return diff.files.find((file) => file.path === path || file.oldPath === path || file.newPath === path) ?? null;
}
function findHunk(file, hunkId) {
    return (file.hunks.find((hunk) => hunk.id === hunkId || `${file.path}:${hunk.oldStart}:${hunk.newStart}` === hunkId) ?? null);
}
