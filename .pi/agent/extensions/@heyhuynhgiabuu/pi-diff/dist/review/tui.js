...
import { countReviewDiffLines, countReviewFileLines, countReviewHunkLines } from "./git.js";
import { getViewportState, moveSelectedPreviewLine, selectPreviewLineForComment, selectPreviewLineForHunk, syncPreviewSelection, } from "./model.js";
import { getSelectedComment, moveSelectedComment, moveSelectedFile, moveSelectedHunk, toggleSelectedCommentStatus, } from "./session.js";
...
export class ReviewDiffPane {
    ...
    requestRender;
    renderFilePreview;
    constructor(diff, session, theme, onDone, options = {}) {
        this.diff = diff;
    ...
    render(width) {
        ...
        const fileWindow = visibleWindow(this.diff.files, indexOfSelectedFile(this.diff.files, viewport.file?.path), FILE_WINDOW_SIZE);
        const hunkWindow = visibleWindow(viewport.file?.hunks ?? [], indexOfSelectedHunk(viewport.file?.hunks ?? [], viewport.hunk?.id), HUNK_WINDOW_SIZE);
        const commentWindow = visibleWindow(this.session.comments, indexOfSelectedComment(this.session.comments, selectedComment?.id), COMMENT_WINDOW_SIZE);
        const row = (content = "") => {
        ...
        const addWrapped = (content, prefix = "") => {
        ...
        row(th.fg("accent", th.bold(" Review Diff ")) +
        ...
        if (fileWindow.start > 0)
            addWrapped(th.fg("muted", `… ${fileWindow.start} earlier files`), "    ");
        for (const file of fileWindow.items) {
            const marker = viewport.file?.path === file.path ? "▶" : " ";
            const fileCounts = countReviewFileLines(file);
            addWrapped(this.styleForFocus("files", viewport.file?.path === file.path
                ? th.fg("accent", `${marker} ${file.path} (${file.status}, +${fileCounts.insertions}/-${fileCounts.deletions}, ${file.hunks.length} hunks)`)
        ...
        if (!viewport.file || viewport.file.hunks.length === 0) {
            ...
            if (hunkWindow.start > 0)
                addWrapped(th.fg("muted", `… ${hunkWindow.start} earlier hunks`), "    ");
            for (const hunk of hunkWindow.items) {
                const marker = viewport.hunk?.id === hunk.id ? "▶" : " ";
                const hunkCounts = countReviewHunkLines(hunk);
                addWrapped(this.styleForFocus("hunks", viewport.hunk?.id === hunk.id
                    ? th.fg("accent", `${marker} ${hunk.id} (+${hunkCounts.insertions}/-${hunkCounts.deletions}) ${hunk.header}`)
        ...
        if (viewport.file) {
            ...
            addWrapped(th.fg("muted", `Fixed ${FILE_DIFF_VIEWPORT_HEIGHT}-line hunk viewport • ${scrollStatus} • mouse wheel/↑↓/pgup/pgdn/home/end scroll`), "    ");
            this.ensurePreview({
            ...
            this.renderFixedPreviewRows(row, th, viewport.windowStart, viewport.windowEnd, viewport.lines.length, viewport.visibleLines, viewport.selectedLine?.id);
        }
        else {
        ...
        if (this.session.comments.length === 0) {
            addWrapped(th.fg("muted", "No drafted comments yet. Press c to draft one on the selected preview line or hunk."), "    ");
        }
        else {
            if (commentWindow.start > 0)
                addWrapped(th.fg("muted", `… ${commentWindow.start} earlier comments`), "    ");
            for (const comment of commentWindow.items) {
                const marker = selectedComment?.id === comment.id ? "▶" : " ";
                const status = comment.status === "dismissed"
        ...
        divider();
        row("");
        addWrapped(th.fg("muted", "tab cycle focus • ↑↓/^j^k move • pgup/pgdn page • home/end jump • [/] prev/next file • o open location • y copy location"), "  ");
        addWrapped(th.fg("muted", "c draft comment • e edit comment • x delete • a approve • d dismiss • A approve all • r refresh • s submit to Pi • esc/q close"), "  ");
        lines.push(clampToWidth(th.fg("border", `╰${"─".repeat(innerW)}╯`), maxW));
    ...
    updateReviewState(diff, session) {
        this.diff = diff;
        this.session = session;
    ...
    renderFixedPreviewRows(row, theme, windowStart, windowEnd, _totalLines, visibleLines, selectedLineId) {
    ...
    applyPreviewSelection(theme, previewLines, visibleLines, selectedLineId) {
        return previewLines.map((line, index) => {
            const viewportLine = visibleLines[index];
            if (!viewportLine?.isSelectable || viewportLine.id !== selectedLineId || line.length === 0)
                return line;
    ...
    movePage(direction) {
        const distance = this.focus === "files"
            ? FILE_WINDOW_SIZE - 1
            : this.focus === "hunks"
                ? HUNK_WINDOW_SIZE - 1
                : this.focus === "preview"
...
function visibleWindow(items, selectedIndex, limit) {
    if (items.length <= limit)
        return { items, start: 0, end: items.length, total: items.length };
...
function indexOfSelectedFile(files, selectedPath) {
    if (!selectedPath)
        return 0;
    const index = files.findIndex((file) => file.path === selectedPath || file.oldPath === selectedPath);
    return index < 0 ? 0 : index;
}
function indexOfSelectedHunk(hunks, selectedId) {
    if (!selectedId)
        return 0;
    const index = hunks.findIndex((hunk) => hunk.id === selectedId);
    return index < 0 ? 0 : index;
}
function indexOfSelectedComment(comments, selectedId) {
    if (!selectedId)
        return 0;
...
function clampToWidth(content, width) {
    return visibleWidth(content) > width ? truncateToWidth(content, width) : content;
}
function expandTabs(content, startColumn = 0) {
    let result = "";
...
function findAnsiSequenceEnd(content, start) {
    let index = start + 1;
...
function mouseWheelDelta(data) {
    if (data === "mouseWheelUp" || data === "wheelUp" || data === "scrollUp")
        return -1;
    if (data === "mouseWheelDown" || data === "wheelDown" || data === "scrollDown")
        return 1;
    if (data.includes("[<64;"))
        return -1;
    if (data.includes("[<65;"))
