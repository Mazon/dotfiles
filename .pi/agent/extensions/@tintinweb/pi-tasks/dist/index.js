...
import { createCadenceState, drainReminderForContext, evaluateToolResult, onTurnStart, resetCadenceState, } from "./reminder-cadence.js";
...
export default function (pi) {
    ...
    pi.on("tool_result", async (event) => {
        ...
        if (!isTaskTool && cadence.reminderInjectedThisCycle)
            return {};
    ...
    pi.on("context", async (event) => {
        if (!drainReminderForContext(cadence))
            return {};
        return {
            messages: [
                ...event.messages,
                {
                    role: "user",
                    content: [{ type: "text", text: SYSTEM_REMINDER }],
                    timestamp: Date.now(),
                },
            ],
        };
