...
- **preflight**: Run pre-flight checks for parallel mode (git repo, clean tree, worktree count)
...
5. Remove: action "remove", name "feature-work"`,
        ...
        args: {
          action: tool.schema
            .enum(["create", "list", "merge", "preflight", "remove", "reset", "prune", "status"])
        ...
        async execute(args, toolCtx): Promise<string> {
          ...
          try {
            switch (args.action) {
              ...
              case "preflight":
                result = await preflightCheck($, repoRoot);
                break;
...
async function mergeWorktree(
  ...
  if (
    ...
    // Target branch is checked out in main worktree.
    // Strategy: merge inside the worktree first, then fast-forward main worktree.

    await log(client, "info", "Target checked out — using fast-forward strategy", {
      sessionID,
      name,
      branch,
      targetBranch,
    ...
    // Merge target INTO worktree branch (inside the worktree)
    let testMerge;

    if (strategy === "manual") {
      testMerge = await $`git merge --no-commit --no-ff ${targetBranch}`
        .quiet()
        .nothrow()
        .cwd(worktreeDir);
    } else if (strategy === "ours") {
      testMerge = await $`git merge -X theirs -m ${message} ${targetBranch}`
        .quiet()
        .nothrow()
        .cwd(worktreeDir);
    } else {
      testMerge = await $`git merge -X ours -m ${message} ${targetBranch}`
        .quiet()
        .nothrow()
        .cwd(worktreeDir);
    }

    const testStdout = getStdout(testMerge);
    const testStderr = getStderr(testMerge);

    if (testMerge.exitCode !== 0) {
      if (testStderr.includes("CONFLICT") || testStdout.includes("CONFLICT")) {
        ...
        return {
          success: false,
          message: `Merge has conflicts. Aborted to keep worktree clean.\n\nConflicted files:\n${conflicts.join("\n")}`,
          conflicts,
          diff: diff || undefined,
        };
      }
      return {
        success: false,
        message: `Merge failed: ${testStderr || testStdout}`,
      };
    }
    ...
    // Fast-forward main worktree to the merge result
    const ffResult = await $`git merge --ff-only ${branch}`
      .quiet()
      .nothrow()
      .cwd(repoRoot);
    ...
    if (ffResult.exitCode !== 0) {
      return {
        success: false,
        message: `Merge succeeded in worktree but fast-forward failed in main worktree: ${getStderr(ffResult)}. The merge commit exists on branch '${branch}'.`,
        branch,
      };
    }

    await log(client, "info", "Merged via fast-forward (target was checked out)", {
      sessionID,
      name,
      branch,
      targetBranch,
    ...
    return {
      success: true,
      message: `Successfully merged '${branch}' into '${targetBranch}' via fast-forward (target was checked out in main worktree)`,
      branch,
    };
  }
...
async function preflightCheck(
  ...
  // Check 1: Git repo
  const repoCheck = await $`git rev-parse --is-inside-work-tree`
    .quiet()
    .nothrow()
    .cwd(repoRoot);
  ...
  if (repoCheck.exitCode !== 0) {
    return {
      success: false,
      message: "Not a git repository. Parallel mode requires a git repo.",
    };
  }
  ...
  if (statusCheck.exitCode === 0 && getStdout(statusCheck)) {
    const dirtyFiles = getStdout(statusCheck).split("\n").length;
    issues.push(
      `Working tree has ${dirtyFiles} uncommitted change(s).\n${getStdout(statusCheck)}`,
    );
  }

  // Check 3: Existing worktree count (max 4)
  ...
  let worktreeCount = 0;
  if (wtList.exitCode === 0) {
    worktreeCount = getStdout(wtList)
      .split("\n")
      .filter((line) => line.startsWith("worktree ")).length;
    if (worktreeCount >= 5) {
      issues.push(
        `Too many worktrees (${worktreeCount}). Maximum is 4 concurrent worktrees.`,
      );
    }
  }
  ...
  return {
    success: true,
    message: `Pre-flight checks passed. Git repo OK, clean working tree, ${worktreeCount} existing worktree(s).`,
  };
}
