---
name: debug
description: Dig into an error or problem, perform root-cause analysis, and create independent board tasks for each fix. Use when the user asks to debug an error, investigate a problem, or troubleshoot an issue.
argument-hint: "The error message, stack trace, or problem description to investigate"
---

# Debug

Investigate an error or problem, trace it to its root causes, and create independent actionable tasks on the board to fix each one.
...
```
Investigate this error/problem and trace it to root causes, then create board tasks for each fix.
...
## Phase 4: Create Board Tasks

For each root cause, create a task using board_create_task:

1. Call board_set_project with a name derived from the debug session if no project is already active.
2. Create one task per root cause with:
   - **Title:** prefixed with [debug] (e.g. "[debug] Add null guard before mapping orders response")
   - **Description:** file path, line range, what's wrong, the concrete fix needed, edge cases, how to verify
   - **Priority:** urgent (prod crash/data loss/security), high (broken feature), medium (degraded/workaround exists), low (cosmetic)
   - **Type:** bug for defects, chore for config/infra issues
...
- "Just the tasks for now" — Stop here. Tasks are on the board for later.

If the user chooses to fix: pick highest-priority first, implement the fix, update task status to code-review, then move to next.
